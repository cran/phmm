/* From wally.gilks@mrc-bsu.cam.ac.uk */
...
/*#define RAND_MAX 2147483647 */
      /* For Sun4 : remove this for some systems */ 
...

{
  ...
  env = (ENVELOPE *)R_alloc(1, sizeof(ENVELOPE));
  if(env == NULL){
    /* insufficient space */
    return 1006;
  }
  ...
  metrop = (METROPOLIS *)R_alloc(1, sizeof(METROPOLIS));
  if(metrop == NULL){
    /* insufficient space */
    return 1006;
  }
...
/*printf("i=%i\n", i);*/
  ...
  /* free space -- Used R_alloc instead
  Free(env->p);
  Free(env);
  Free(metrop); */
...

{
  ...
  *(env->neval) = 0;
  ...
  env->npoint = npoint;
  env->p = (POINT *)R_alloc(npoint, sizeof(POINT));
  if(env->p == NULL){
    /* insufficient space */
    return 1006;
  }
