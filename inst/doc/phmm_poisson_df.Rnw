...
\begin{document}
\SweaveOpts{engine=R, eps=FALSE, pdf=TRUE, strip.white=TRUE, echo=TRUE}
...
We discuss connections between the Cox proportional hazards model and Poisson generalized linear models as described in Whitehead (1980). We fit a sample dataset using coxph() and glm() and show that the model degrees of freedom differ by the number of events.
